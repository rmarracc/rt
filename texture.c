...
int				init_textures(t_instance *inst)
{
	...
	env = inst->env;
	s = &inst->scn;
	s->tex[0] = mlx_xpm_file_to_image(env->mlx, "nord.xpm", s->w + 0, s->h + 0);
	s->tex[1] = mlx_xpm_file_to_image(env->mlx, "sud.xpm", s->w + 1, s->h + 1);
	s->tex[2] = mlx_xpm_file_to_image(env->mlx, "est.xpm", s->w + 2, s->h + 2);
	s->tex[3] = mlx_xpm_file_to_image(env->mlx, "ou.xpm", s->w + 3, s->h + 3);
	s->tex[4] = mlx_xpm_file_to_image(env->mlx, "haut.xpm", s->w + 4, s->h + 4);
	s->tex[5] = mlx_xpm_file_to_image(env->mlx, "bas.xpm", s->w + 5, s->h + 5);
	while (k < 6)
		if (s->tex[k++] == NULL)
			return (ERROR);
