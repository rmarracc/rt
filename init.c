...
void			init_hooks(t_environment *env)
{
	...
	while (pinst != NULL)
	{
		...
		mlx_hook(pinst->win.win, KEY_PRESS, 0, key_press, pinst);
		mlx_hook(pinst->win.win, KEY_RELEASE, 0, key_release, pinst);
		mlx_hook(pinst->win.win, MOUSE_PRESS, 0, mouse_press, pinst);
		mlx_hook(pinst->win.win, MOUSE_RELEASE, 0, mouse_release, pinst);
		mlx_hook(pinst->win.win, MOUSE_MOVE, 0, mouse_move, pinst);
		mlx_hook(pinst->win.win, EXPOSE_EVENT, 0, expose_event, pinst);
		mlx_hook(pinst->win.win, CLOSE_EVENT, 0, close_event, pinst);
		pinst = pinst->next;
	}
